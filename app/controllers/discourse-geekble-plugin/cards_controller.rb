module DiscourseGeekblePlugin
  class CardsController < ApplicationController
    ...
    def card_topics(page, pin = nil)
      ...
      stmt = Topic.includes(:tags).includes(posts: [:user, :post_actions])
    ...
    def topic_to_card(c)
      ...
      remarkable_posts = c.posts.where("post_number != 1").where("like_count > 0").order('like_count desc').limit(1)
      ...
      posts_thread = c.posts.order(created_at: :asc)
      fp = posts_thread.first
      first_post = fp.as_json
      if current_user
        liked_by_me = {liked_by_me: !!fp.post_actions.find_by(user_id: current_user.id, post_action_type_id: 2)}.as_json
